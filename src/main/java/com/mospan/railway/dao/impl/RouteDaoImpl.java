...
public class RouteDaoImpl implements RouteDao {
    ...
    public void insert(Route route) {
        ...
        try (PreparedStatement st = con.prepareStatement("INSERT INTO route (start_station_id, end_station_id, depart_time, time, price)" +
                " VALUES (?, ?, ?, ?, ?)")){
    ...
    public void update(Route route) {
        con = cp.getConnection();
        try (PreparedStatement st = con.prepareStatement("UPDATE route SET start_station_id = ?, end_station_id = ?, depart_time = ?, " +
                "time = ?, price = ? WHERE id = ?")) {
    ...
    public Route findById(long id) {
        ...
        try (PreparedStatement st = con.prepareStatement("SELECT * FROM route WHERE id = ?")){
    ...
    public void delete(Route route) {
        ...
        con = cp.getConnection();
        try ( PreparedStatement st = con.prepareStatement("DELETE FROM route WHERE id = ?")) {
    ...
    public Collection<Route> findAll() {
        ...
        try ( PreparedStatement st = con.prepareStatement("SELECT * FROM route")){
    ...
    public Collection<Route> findByStations(String startStation, String endStation) {
        ...
        try (PreparedStatement st = con.prepareStatement("SELECT * FROM route WHERE start_station_id = ? " +
                "AND end_station_id = ?")){
    ...
    public Collection<Route> findByStation(Station station) {
        ...
        try ( PreparedStatement st = con.prepareStatement("SELECT * FROM route WHERE start_station_id = ? UNION SELECT * FROM route WHERE end_station_id = ?")) {
    ...
    public long findByParams(Station s1, Station s2, LocalTime depTime, long time) {
        con = cp.getConnection();
        long id = 0;
        try (PreparedStatement st = con.prepareStatement("SELECT id FROM route WHERE start_station_id = ? AND end_station_id = ? AND " +
