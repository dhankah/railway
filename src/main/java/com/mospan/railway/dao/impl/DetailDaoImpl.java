...
public class DetailDaoImpl implements DetailDao {
    private static final Logger logger = Logger.getLogger(DetailDaoImpl.class);
    ...
    public void insert(Detail detail) {
        ...
        try (PreparedStatement st = con.prepareStatement("INSERT INTO detail (first_name, last_name, email) VALUES (?, ?, ?)")) {
    ...
    public void update(Detail detail) {
        con = cp.getConnection();
        try (PreparedStatement st = con.prepareStatement("UPDATE detail " +
    ...
    public Detail findById(long id) {
        ...
        try (PreparedStatement st = con.prepareStatement("SELECT * FROM detail WHERE id = ?")) {
    ...
    public void delete(Detail detail) {
        con = cp.getConnection();
        try (PreparedStatement st = con.prepareStatement("DELETE FROM detail WHERE id = ?")){
    ...
    public Collection<Detail> findAll() {
        ...
        try (PreparedStatement st = con.prepareStatement("SELECT * FROM detail")){
            ResultSet rs = st.executeQuery();
            long id = 1;
            while (rs.next()) {
